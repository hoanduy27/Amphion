...
# Inference
adv_dir = "egs/tts/VALLE/adv_gdba"
inference_dir = 'egs/tts/VALLE/adv_gdba_inference'
...
audio = valle_inference.inference_one_clip(
    text = "As the sun dipped belown the horizon",
    text_prompt =  "But even the unsuccessful dramatist has his moments",
    audio_file = f"egs/tts/VALLE/prompt_examples/7176_92135_000004_000000.wav",
)

sf.write(
    os.path.join(inference_dir, f'output_origin.wav'), 
    audio, samplerate=valle_inference.audio_tokenizer.sample_rate
)

for i in range(10):
    # Get adv sample
    sample = valle_inference.sample(
        coeff, 
        "egs/tts/VALLE/prompt_examples/7176_92135_000004_000000.wav",
    )
    
    sf.write(
        os.path.join(adv_dir, f'output_{str(i)}.wav'), 
        sample, samplerate=valle_inference.audio_tokenizer.sample_rate
    )
    ...
    audio = valle_inference.inference_one_clip(
        text = "As the sun dipped belown the horizon",
        text_prompt =  "But even the unsuccessful dramatist has his moments",
        audio_file = f"egs/tts/VALLE/adv/output_{str(i)}.wav",
    )
